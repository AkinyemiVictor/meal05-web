...
revoke execute on function public.clear_order_active_delivery_route() from public, anon, authenticated;
...
for each row execute function public.clear_order_active_delivery_route();
...
revoke execute on function public.prevent_duplicate_active_route_stop() from public, anon, authenticated;
...
for each row execute function public.prevent_duplicate_active_route_stop();

drop function if exists public.create_delivery_route_transaction(
  uuid, integer[], uuid, text, timestamptz, text, numeric, numeric,
...
begin
  ...
  insert into public.delivery_routes (
    delivery_partner_id, status, vehicle_type, planned_start_time,
    pickup_location, agreed_partner_payment, delivery_fees_collected,
    other_delivery_cost, notes, created_by
  ) values (
    p_delivery_partner_id, 'assigned', coalesce(p_vehicle_type, v_partner.vehicle_type),
    p_planned_start_time, nullif(trim(coalesce(p_pickup_location, '')), ''),
    p_agreed_partner_payment, v_delivery_fees, coalesce(p_other_delivery_cost, 0),
  ...
  loop
    ...
    insert into public.delivery_route_stops (
      route_id, order_id, stop_number, customer_id, customer_name,
      customer_phone, delivery_address, delivery_landmark, status,
      delivery_otp_hash, otp_expires_at, delivery_notes,
      delivery_latitude, delivery_longitude
    ) values (
      v_route.id, v_order.id, v_stop_number, v_order.user_id,
      coalesce(nullif(v_order.delivery_contact_name, ''), 'Meal05 customer'),
      coalesce(nullif(v_order.delivery_contact_phone, ''), 'Not provided'),
      coalesce(nullif(v_order.delivery_address, ''),
        nullif(concat_ws(', ', nullif(v_order.delivery_house_number, ''), nullif(v_order.delivery_street, ''), nullif(v_order.delivery_landmark, '')), ''),
  ...
  insert into public.delivery_access_tokens (
    route_id, delivery_partner_id, token_hash, pin_hash, expires_at
  ) values (
    v_route.id, p_delivery_partner_id, v_token_hash, v_pin_hash, v_token_expires_at
  ) returning id into v_access_token_id;

  insert into public.delivery_audit_logs (
    route_id, actor_user_id, actor_type, action, new_value, ip_address, user_agent
  ) values (
    v_route.id, v_actor, 'dispatcher', 'route_created',
    jsonb_build_object(
      'route_id', v_route.id,
  ...
  insert into public.delivery_audit_logs (
    ...
    v_route.id, v_actor, 'dispatcher', 'token_generated',
