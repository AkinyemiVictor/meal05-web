alter table public.products
  ...
  add column if not exists display_group_order integer;

comment on column public.products.display_group is 'Optional storefront grouping key used to keep sibling product sizes adjacent.';
comment on column public.products.display_group_order is 'Order of a product within its storefront display group.';
...
select
    ...
    (coalesce(chosen_variant.stock_count, 0::numeric) > 0::numeric) as in_stock,
    pm.market_id,
    ...
    case
      when nullif(btrim(p.display_group), '') is null then p.id * 100
...
join lateral (
    ...
    order by (coalesce(v.stock_count, 0::numeric) > 0::numeric) desc, v.price, v.id
    limit 1
