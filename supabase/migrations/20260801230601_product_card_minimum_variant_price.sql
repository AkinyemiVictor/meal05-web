-- Product cards use the cheapest valid in-stock option for the current market.
-- If every valid option is depleted, they use the cheapest active option.

create index if not exists product_variants_card_price_lookup_idx
  on public.product_variants (product_id, market_id, price, stock_count, id)
