-- Remove the obsolete product-level stock trigger. Stock belongs to product_variants.stock_count.
...
-- Old payment-status triggers bypass the explicit, atomic payment RPCs.
...
-- Align the persisted cart with the variant-based checkout code.
...
-- Expand the legacy payments table for Phase 1 manual transfers and wallet deposits.
...
-- Allow the Phase 1 transfer provider/status values used by deployed routes.
...
-- One stock deduction ledger row per order/variant prevents repeat deductions.
...
begin
  ...
  -- Lock and validate every variant in deterministic order before changing any stock.
  for v_item in
