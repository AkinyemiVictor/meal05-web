...
import { regenerateStopOtp } from "@/lib/delivery/management";
...
export async function POST(req, { params }) {
  const rl = await checkRateLimit({ request: req, id: "delivery:stops:regenerate-otp", limit: 20, windowMs: 60_000 });
  ...
  try {
    const result = await regenerateStopOtp({
      stopId,
    ...
    return send(NextResponse.json({ error: error.message || "Unable to regenerate OTP." }, { status: 400 }), rl);
  }
}
