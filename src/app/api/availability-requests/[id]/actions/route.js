...
export async function POST(request, { params }) {
  ...
  if (parsed.data.action === "remove_unavailable_item") {
    ...
    await admin.from("availability_request_items").update({ customer_removed_at: new Date().toISOString(), updated_at: new Date().toISOString() })
      .eq("id", item.id).eq("request_id", id);
    const remaining = (availabilityRequest.items || []).map((entry) => entry.id === item.id ? { ...entry, customer_removed_at: new Date().toISOString() } : entry);
    const status = resolveRequestState(remaining);
    const total = calculateRequestTotal(remaining);
    const confirmed = status === "confirmed";
    const now = new Date();
    const { data: updated } = await admin.from("availability_requests").update({
