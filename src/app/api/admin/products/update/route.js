...
export async function POST(req) {
  ...
  if (!hasSeason && !hasPrice && !hasOldPrice && !hasPromoText && !hasPromoExpiry && !hasPromoEnabled) {
    return applyRateLimitHeaders(NextResponse.json({ error: "No update fields provided" }, { status: 400 }), rl);
  }
  if ((hasPrice || hasOldPrice) && !variantId) {
  ...
  const [productRes, variantRes] = await Promise.all([
    admin
      .from("products")
      .select("id, name, in_season, promo_tag_text, promo_tag_expires_at, promo_tag_enabled")
      .eq("id", productId)
      .maybeSingle(),
    variantId
      ? admin.from("product_variants").select("id, product_id, name, price, old_price").eq("id", variantId).maybeSingle()
  ...
  if (variantId && Object.keys(variantPatch).length) {
    const result = await admin
      ...
      .update(variantPatch)
      .eq("id", variantId)
      .select("id, product_id, name, price, old_price")
      .maybeSingle();
    if (result.error) {
