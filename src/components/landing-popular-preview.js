...
export default function LandingPopularPreview() {
  ...
  return <HomeProductCollection eyebrow="Popular right now" title="Fresh picks of the day" products={popular} status={status} emptyMessage="Fresh picks are being restocked." seeAllHref="/section/popular" onAdd={(product)=>router.push(getProductHref(product))} actionLabel="View product" preserveSingleRow />;
}
