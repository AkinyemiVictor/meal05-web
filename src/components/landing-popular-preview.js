...
export default function LandingPopularPreview() {
  ...
  const { ordered: products, status } = useCatalogProducts("/api/catalog/home?limit=24");
  ...
  return <HomeProductCollection eyebrow="Popular right now" title="Fresh picks of the day" products={popular} status={status} emptyMessage="Fresh picks are being restocked." seeAllHref="/section/popular" onAdd={(product)=>router.push(getProductHref(product))} actionLabel="View product" variant="landing" />;
}
